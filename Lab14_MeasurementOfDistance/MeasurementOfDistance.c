...
void SysTick_Init(unsigned long period){
	...
	SYSCTL_RCGC2_R |= 0x00000020;   //active PORTA clock
	delay = SYSCTL_RCGC2_R;         //wait 3-5 bus cycle
	GPIO_PORTF_AMSEL_R &=~0x02;     //no analog
	GPIO_PORTF_PCTL_R &= ~0x0000000F; //regular function
	GPIO_PORTF_DIR_R |= 0x02;         //PA2 output
	//GPIO_PORTA_DR8R_R |= 0x04;       //can drive up to 8mA for output PA2
	GPIO_PORTF_AFSEL_R &= ~0x02;     //disable alternate function
	GPIO_PORTF_DEN_R |= 0x02;        //enable digital I/O device
	NVIC_ST_CTRL_R = 0;
...
// initialize profiling on PF1 (optional)
  GPIO_PORTF_DATA_R |=0x02;                               //    wait for clock to stabilize
  EnableInterrupts();
