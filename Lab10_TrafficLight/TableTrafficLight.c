...
struct State{
	unsigned long TrafficLed; // output for traffic lights (portB)
	unsigned long BoardLed; // output for pedestrian lights (portF)
...
int main(void){ 
	...
	portBEF_Init(); // Initialise port B,E,F
	SysTick_Init(); // Initialise systick
  EnableInterrupts();
	
	// Looping through FSM
  while(1){
    TrafficLight = FSM[S].TrafficLed;  // set traffic lights
		WalkLight = FSM[S].BoardLed; // set pedestrian lights
    SysTick_Wait(FSM[S].Time); // delay
    S = FSM[S].Next[Sensor];  // next state
  }
}
